...
export class CBStatementComponent implements OnInit {
    ...
    this.restApiService.get(PathConstants.CB_STATEMENT_REPORT).subscribe(response => {
      ...
      for(let i = 0; i < this.cbData.length; i++) {
        if (this.cbData[i].RGNAME !== groupedData[index].RGNAME) {
            item = { TNCSName: 'TOTAL', TNCSCapacity: groupedData[index].TNCSCapacity,
  ...
  exportAsXLSX(): void {
    var closing_balance_data = [];
    this.cbData.forEach(data => {
      closing_balance_data.push({TNCSName: data.TNCSName, TNCSCapacity: data.TNCSCapacity,
    ...
    this.excelService.exportAsExcelFile(closing_balance_data, 'CLOSING_BALANCE_STATEMENT_REPORT', this.column);
  }
