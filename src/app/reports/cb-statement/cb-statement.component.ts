...
Rx.Observable.from(this.cbData)
      ...
      for(let i = 0; i < this.cbData.length; i++) {
        if (this.cbData[i].RGNAME !== groupedData[index].RGNAME) {
            item = { TNCSName: 'TOTAL', TNCSCapacity: groupedData[index].TNCSCapacity, boiledRice: (groupedData[index].boiledRice * 1).toFixed(3), rawRice: (groupedData[index].rawRice * 1).toFixed(3),
            totalRice: (groupedData[index].totalRice * 1).toFixed(3), totalDhall: (groupedData[index].totalDhall * 1).toFixed(3), toorDhall: (groupedData[index].toorDhall * 1).toFixed(3), kanadaToorDhall: (groupedData[index].kanadaToorDhall * 1).toFixed(3),
          kanadaToorDhallTotal: (groupedData[index].kanadaToorDhallTotal * 1).toFixed(3), SUGAR: (groupedData[index].SUGAR * 1).toFixed(3), WHEAT: (groupedData[index].WHEAT * 1).toFixed(3), cement: (groupedData[index].cement * 1).toFixed(3),
        uridDhall: (groupedData[index].uridDhall * 1).toFixed(3), palmoil: groupedData[index].palmoil };
          this.cbData.splice(i, 0, item);
          index += 1;
           console.log(this.cbData.length);
        //   }
        }
      }
