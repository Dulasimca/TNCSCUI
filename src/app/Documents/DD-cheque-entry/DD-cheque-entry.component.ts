...
export class DDChequeEntryComponent implements OnInit {
  ...
  onView() {
    ...
    const params = new HttpParams().set('GCode', this.GCode).append('Type', '1').append('value', this.datepipe.transform(this.viewDate, 'MM/dd/yyyy'));
  ...
  onClear() {
    ...
    this.chequeAmount = 0; this.chequeDate = new Date();
    this.chequeNo = null; this.bank = null; this.receivedFrom = null;
  }

  onRowSelect(event) {
    // this.rowId = event.data.RowId;
    this.receiptNo = event.data.vrno;
  }

  getDocByReceiptNo() {
    this.viewPane = false;
    const params = new HttpParams().set('value', this.receiptNo).append('Type', '2');
    this.restApiService.getByParameters(PathConstants.DD_CHEQUE_ENTRY_GET, params).subscribe((res: any) => {
      if (res !== undefined && res !== null && res.length !== 0) {
        // this.ChequeReceiptNoData = res;
        res.forEach(x => {
          x.ChequeDate = this.datepipe.transform(x.ChequeDate, 'dd/MM/yyyy');
        })
        this.DDChequeData = res;
      } else {
        this.messageService.add({ key: 't-err', severity: StatusMessage.SEVERITY_ERROR, summary: StatusMessage.SUMMARY_ERROR, detail: StatusMessage.ErrorMessage });
      }
    }, (err: HttpErrorResponse) => {
  ...
  viewSelectedRow(data, index) {
    ...
    if(data.PayName !== undefined && data.PayName !== null) {
      this.paymentType = data.PayName;
      // this.paymentTypeOptions = [{ label: data.PayName, value: data.PaymentType }];
    } else {
    let paymentType;
    this.paymentTypeOptions.filter(x => {
      if(data.PaymentType === x.value)
      paymentType = x.label;
    })
    this.paymentType = paymentType;
    // this.paymentTypeOptions = [{ label: paymentType, value: data.PaymentType }];
  }
    this.bank = data.Bank;
    ...
    this.receiptDate = (data.ReceiptDate !== undefined && data.ReceiptDate !== null) ? data.ReceiptDate : '-';
